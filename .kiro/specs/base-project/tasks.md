...
- [ ] 8. Checkpoint — verify auth flow compiles and middleware works end-to-end
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Shared UI components
  - [ ] 9.1 Create `src/components/ui/Button.tsx`
    - Props: `variant: "primary" | "secondary" | "destructive"`, `isLoading?: boolean`, plus all standard `ButtonHTMLAttributes`
    - Tailwind-based styling per variant; shows a spinner or disabled state when `isLoading` is true
    - _Requirements: FR-07_
  - [ ] 9.2 Create `src/components/ui/Input.tsx`
    - Props: `label: string`, `error?: string`, `id: string`, plus all standard `InputHTMLAttributes`
    - Renders `<label>` above and an error message below when `error` is set
    - _Requirements: FR-07_
  - [ ] 9.3 Create `src/components/ui/FormMessage.tsx`
    - Props: `type: "success" | "error"`, `message: string`
    - Renders a styled block with appropriate color per type
    - _Requirements: FR-07_
  - [ ] 9.4 Create `src/components/layout/Navbar.tsx` as a Server Component
    - Call `createClient()` from `lib/supabase/server.ts` to get the current user
    - Show app name/logo always; show sign-out form button only when a user is authenticated
    - Sign-out button submits to `signOutAction` via a `<form action={signOutAction}>`
    - No `"use client"` directive
    - _Requirements: FR-07_
  - [ ] 9.5 Create `src/components/layout/Sidebar.tsx` as a Server Component
    - Renders navigation links for the dashboard section
    - No `"use client"` directive
    - _Requirements: FR-07_

- [ ] 10. Auth pages
  - [ ] 10.1 Create `src/app/(auth)/layout.tsx` — centered card layout, no sidebar, minimal header with app name
    - _Requirements: FR-04, FR-07_
  - [ ] 10.2 Create `src/app/(auth)/login/page.tsx` as a Server Component
  ...
  - [ ] 10.3 Create `src/app/(auth)/register/page.tsx` as a Server Component
    - Plain HTML `<form action={signUpAction}>` with email, password, confirmPassword inputs
    - Renders `FormMessage` for success or error
    - _Requirements: FR-03, FR-04, NFR-03_
  - [ ] 10.4 Create `src/app/(auth)/reset-password/page.tsx` as a Server Component
    - Plain HTML `<form action={resetPasswordAction}>` with email input
    - Renders `FormMessage` for success or error
    - _Requirements: FR-03, FR-04, NFR-03_

- [ ] 11. Dashboard pages
  - [ ] 11.1 Create `src/app/(dashboard)/layout.tsx` as a Server Component
  ...
  - [ ] 11.2 Create `src/app/(dashboard)/dashboard/page.tsx` as a Server Component
    - Call `requireUser()` and display a welcome message with the user's email
    - _Requirements: FR-04_
  - [ ] 11.3 Create `src/app/(dashboard)/dashboard/profile/page.tsx` as a Server Component
    - Call `requireUser()` and display the user's email, ID, and `created_at` (formatted as ISO string for serialization)
    - _Requirements: FR-04_

- [ ] 12. Root layout and root page
  - [ ] 12.1 Update `src/app/layout.tsx` — root layout with `<html>`, `<body>`, `globals.css` import, and `<Navbar />` rendered at the root level
    - _Requirements: FR-07_
  - [ ] 12.2 Update `src/app/globals.css` — Tailwind directives (`@tailwind base/components/utilities`) and CSS custom properties for the color palette
    - _Requirements: FR-07_
  - [ ] 12.3 Update `src/app/page.tsx` — check session server-side via `createClient()`, redirect to `/dashboard` if authenticated, redirect to `/login` if not
    - _Requirements: FR-04_

- [ ] 13. Final checkpoint — full build and integration verification
  - Ensure all tests pass, ask the user if questions arise.
