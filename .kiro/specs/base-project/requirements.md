...
- [ ] `pnpm run dev` starts the development server with no runtime errors.
- [ ] `pnpm run build` produces a successful production build with no TypeScript errors.
- [ ] A user can sign up, receive a confirmation email, sign in, and access `/dashboard`.
- [ ] Accessing `/dashboard` without a session redirects to `/login`.
- [ ] Accessing `/login` with an active session redirects to `/dashboard`.
- [ ] Sign-out clears the session and redirects to `/login`.
- [ ] No secrets appear in client-side bundles or browser-visible source.
- [ ] All Supabase clients use the correct context (browser, server, middleware).
