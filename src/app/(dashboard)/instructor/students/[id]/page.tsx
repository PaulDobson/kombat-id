...
export default async function InstructorStudentDetailPage({
  ...
  const [practitioner, certifications, historyEntries, gradeExams] =
    await Promise.all([
      practitionerRepo.findById(id),
      certRepo.findByPractitioner(id),
      historyRepo.findByPractitionerId(id),
      gradeExamRepo.findByPractitioner(id),
    ]);

  if (!practitioner) notFound();

  // Verify this student belongs to the instructor (direct or via academy)
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="bg-neutral-900 border border-neutral-700 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-neutral-700 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-neutral-50">
            Exámenes de grado ({gradeExams.length})
          </h2>
          <Link
            href={`/instructor/grade-exams/new?practitionerId=${id}`}
        ...
        {gradeExams.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-neutral-500 text-sm">
              Sin exámenes de grado registrados.
            </p>
          </div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-neutral-800">
              {gradeExams.map((exam) => (
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${EXAM_STATUS_STYLES[exam.status] ?? "bg-neutral-800 text-neutral-400 border border-neutral-700"}`}
                    >
                  ...
                  <td className="px-5 py-3 text-xs hidden sm:table-cell">
                    <span
                      className={
                        RESULT_STYLES[exam.finalResult] ?? "text-neutral-400"
                      }
                    >
