...
import { deletePractitionerAction } from "@/modules/practitioner-identity/presentation/actions/adminActions";
...
export function DeletePractitionerButton({
  publicId,
  practitionerName,
}: Props) {
  ...
  async function handleDelete() {
    const result = await Swal.fire({
      ...
      html: `
        <p class="text-sm text-neutral-300 mb-3">
          Estás a punto de <strong class="text-red-400">desactivar permanentemente</strong> a <strong>${practitionerName}</strong>.
        </p>
        <div class="text-xs text-neutral-400 text-left bg-neutral-800 p-3 rounded-lg mb-3">
          <p class="font-semibold text-amber-400 mb-2">⚠️ Esta acción desactivará el practicante y:</p>
          ...
          <p class="mt-2 text-blue-400 text-xs">
            <strong>Nota:</strong> Los datos históricos (exámenes, eventos, pagos, historial marcial) se conservarán para auditoría.
          </p>
        </div>
    ...
    startTransition(async () => {
      ...
      if (deleteResult.success) {
        await Swal.fire({
          title: "¡Eliminado!",
          text: "El practicante ha sido desactivado del sistema. Su cuenta de autenticación ha sido eliminada.",
          icon: "success",
  ...
  return (
    ...
    >
      {isPending ? "Eliminando..." : "Eliminar practicante"}
    </button>
  );
}
