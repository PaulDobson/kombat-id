...
import { rejectRefereeRegistrationAction } from "../actions/adminRefereeActions";

interface Props {
  id: string;
}

export function RejectRegistrationButton({ id }: Props) {
  ...
  function handleReject() {
    ...
    startTransition(async () => {
      const result = await rejectRefereeRegistrationAction({ id });
      if (!result.success) {
